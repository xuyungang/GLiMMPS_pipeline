...
cat("You are running sQTLregress.oneexon.parallel.R\n")
...
write.table(tmpout  , paste(TMPASSODIR,"/chr",chr,"/",targetexonID,".1exon.parallel.asso",sep=""),row.names=F,col.names=T, quote=F,sep="\t" )
...
pvalfile <- paste(TMPASSODIR,"/chr",chr,"/",targetexonID,".1exon.parallel.asso",sep="")
