...
## $3 [optional] the number threads for parallel running; defualt = 100. A bigger number may require more memory (100 threads need ~50GB).
##-------------Usage-----------
...
exonInfo=alltype/bychrs/exonsinfor.plink.5reads.chr$1.txt
allJunction=alltype/bychrs/plink.5reads.allreads.chr$1.txt
InclJunction=alltype/bychrs/plink.5reads.IJ.chr$1.txt
genotype=../SNPs_plink/$2.plink.$1
...
echo 
Rscript /mnt/isilon/xing_lab/xuy5/GLIMMPS/Geuvadis/GLiMMPScode/Rscripts/sQTLregress.oneexon.parallel.R $exonInfo $allJunction $InclJunction $genotype $i
done
